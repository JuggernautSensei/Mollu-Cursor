...
struct ImVec2
{
    ...
    constexpr ImVec2(const float _x, const float _y)
        : x(_x)
        , y(_y)
    {
    }
...
struct ImVec4
{
    ...
    constexpr ImVec4(const float _x, const float _y, const float _z, const float _w)
        : x(_x)
        , y(_y)
        , z(_z)
        , w(_w)
...
//   for each static/DLL boundary you are calling from. Read "Application and Memory Allocators" section of imgui.cpp for details.
...
IMGUI_API void             NewFrame();        // m_start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
...
// Windows
// - Begin() = push window to the stack and m_start appending to it. End() = pop window from the stack.
...
//  - BeginPopup(): query popup state, if open m_start appending into the window. Call EndPopup() afterwards if returned true. ImGuiWindowFlags are forwarded to the window.
//  - BeginPopupModal(): block every interaction behind the window, cannot be closed by user, add a dimming background, has a title bar.
IMGUI_API bool BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0);                           // return true if the popup is open, and you can m_start outputting to it.
IMGUI_API bool BeginPopupModal(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);   // return true if the modal is open, and you can m_start outputting to it.
...
// - 5. Populate contents:
//    - In most situations you can use TableNextRow() + TableSetColumnIndex(N) to m_start appending into a column.
...
IMGUI_API void LogToTTY(int auto_open_depth = -1);                                 // m_start logging to tty (stdout)
IMGUI_API void LogToFile(int auto_open_depth = -1, const char* filename = NULL);   // m_start logging to file
IMGUI_API void LogToClipboard(int auto_open_depth = -1);                           // m_start logging to OS clipboard
...
// - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
//   for each static/DLL boundary you are calling from. Read "Application and Memory Allocators" section of imgui.cpp for more details.
...
#    ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT    = ImGuiKey_NamedKey_END,   // Obsoleted in 1.91.5 because it was extremely misleading (since named keys don't m_start at 0 anymore)
...
struct ImVector
{
    ...
    inline int _grow_capacity(const int sz) const
    {
    ...
    inline void resize(const int new_size, const T& v)
    {
        if (new_size > Capacity) reserve(_grow_capacity(new_size));
        if (new_size > Size)
    ...
    inline void shrink(const int new_size)
    {
        IM_ASSERT(new_size <= Size);
        Size = new_size;
    }   // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    inline void reserve(const int new_capacity)
    {
    ...
    inline void reserve_discard(const int new_capacity)
    {
        if (new_capacity <= Capacity) return;
        if (Data) IM_FREE(Data);
...
struct ImGuiIO
{
    ...
    IMGUI_API void ClearEventsQueue();                                                                                           // ClearField all incoming events.
    IMGUI_API void ClearInputKeys();                                                                                             // ClearField current keyboard/gamepad state + current frame text input buffer. Equivalent to releasing all keys/buttons.
    IMGUI_API void ClearInputMouse();                                                                                            // ClearField current mouse state.
#    ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void ClearInputCharacters();   // [Obsoleted in 1.89.8] ClearField the current frame text input buffer. Now included within ClearInputKeys().
#    endif
...
struct ImGuiStoragePair
{
    ...
    ImGuiStoragePair(const ImGuiID _key, const int _val)
    {
        key   = _key;
        val_i = _val;
    }
    ImGuiStoragePair(const ImGuiID _key, const float _val)
    {
        key   = _key;
        val_f = _val;
    }
    ImGuiStoragePair(const ImGuiID _key, void* _val)
    {
        key   = _key;
        val_p = _val;
    }
...
struct ImGuiListClipper
{
    ...
    inline void    IncludeItemByIndex(const int item_index) { IncludeItemsByIndex(item_index, item_index + 1); }
...
#    ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline void IncludeRangeByIndices(const int item_begin, const int item_end) { IncludeItemsByIndex(item_begin, item_end); }        // [renamed in 1.89.9]
    inline void ForceDisplayRangeByIndices(const int item_begin, const int item_end) { IncludeItemsByIndex(item_begin, item_end); }   // [renamed in 1.89.6]
...
struct ImColor
{
    ...
    constexpr ImColor() {}
    constexpr ImColor(const float r, const float g, const float b, const float a = 1.0f)
        : Value(r, g, b, a)
    {
    }
    ...
    constexpr ImColor(const int r, const int g, const int b, const int a = 255)
        : Value((float)r * (1.0f / 255.0f), (float)g * (1.0f / 255.0f), (float)b * (1.0f / 255.0f), (float)a * (1.0f / 255.0f))
    {
    }
    constexpr ImColor(const ImU32 rgba)
    ...
    inline void SetHSV(const float h, const float s, const float v, const float a = 1.0f)
    {
        ImGui::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z);
        Value.w = a;
    }
    static ImColor HSV(const float h, const float s, const float v, const float a = 1.0f)
    {
        float r, g, b;
...
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_ClearOnEscape         = 1 << 9,    // ClearField selection when pressing Escape while scope is focused.
    ImGuiMultiSelectFlags_ClearOnClickVoid      = 1 << 10,   // ClearField selection when clicking on empty location within scope.
...
{
    //------------------------------------------// BeginMultiSelect / EndMultiSelect
    ImGuiSelectionRequestType Type;             //  ms:w, app:r     /  ms:w, app:r   // Request type. You'll most often receive 1 ClearField + 1 SetRange with a single-item range.
...
struct ImGuiSelectionBasicStorage
{
    ...
    IMGUI_API void Clear();                                                                        // ClearField selection
    ...
    inline ImGuiID GetStorageIdFromIndex(const int idx) { return AdapterIndexToStorageId(this, idx); }   // Convert index to item id based on provided adapter.
};
...
struct ImDrawList
{
    ...
    inline void PathFillConvex(const ImU32 col)
    {
        AddConvexPolyFilled(_Path.Data, _Path.Size, col);
        _Path.Size = 0;
    }
    inline void PathFillConcave(const ImU32 col)
    {
        AddConcavePolyFilled(_Path.Data, _Path.Size, col);
        _Path.Size = 0;
    }
    inline void PathStroke(const ImU32 col, const ImDrawFlags flags = 0, const float thickness = 1.0f)
    {
    ...
    inline void ChannelsSplit(const int count) { _Splitter.Split(this, count); }
    inline void ChannelsMerge() { _Splitter.Merge(this); }
    inline void ChannelsSetCurrent(const int n) { _Splitter.SetCurrentChannel(this, n); }
    ...
    inline void    PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, const ImU32 col)
    {
    ...
    inline void PrimWriteIdx(const ImDrawIdx idx)
    {
        *_IdxWritePtr = idx;
        _IdxWritePtr++;
    }
    inline void PrimVtx(const ImVec2& pos, const ImVec2& uv, const ImU32 col)
    {
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool GetBit(const size_t n) const
    {
        int   off  = (int)(n >> 5);
        ImU32 mask = 1u << (n & 31);
        return (UsedChars[off] & mask) != 0;
    }   // Get bit n in the array
    inline void SetBit(const size_t n)
    {
        int   off  = (int)(n >> 5);
        ImU32 mask = 1u << (n & 31);
        UsedChars[off] |= mask;
    }   // Set bit n in the array
    inline void    AddChar(const ImWchar c) { SetBit(c); }                         // Add character
...
struct ImFontAtlas
{
    ...
    IMGUI_API void    ClearInputData();                                                                                                                                                                              // ClearField input data (all ImFontConfig structures including sizes, TTF data, glyph ranges, etc.) = all the data used to build the texture and fonts.
    IMGUI_API void    ClearFonts();                                                                                                                                                                                  // ClearField input+output font data (same as ClearInputData() + glyphs storage, UV coordinates).
    IMGUI_API void    ClearTexData();                                                                                                                                                                                // ClearField output texture data (CPU side). Saves RAM once the texture has been copied to graphics memory.
    IMGUI_API void    Clear();                                                                                                                                                                                       // ClearField all input and output.
    ...
    ImFontAtlasCustomRect* GetCustomRectByIndex(const int index)
    {
        IM_ASSERT(index >= 0);
...
struct ImFont
{
    ...
    float                  GetCharAdvance(const ImWchar c) { return ((int)c < IndexAdvanceX.Size) ? IndexAdvanceX[(int)c] : FallbackAdvanceX; }
...
enum ImGuiViewportFlags_
{
    ...
    // user extension flags
    ImGuiViewportFlags_NoBlockInputs_UserExtension = 1 << 14   // 뷰포트 윈도우가 입력을 막지 않습니다. 오버레이 혹은 HUD를 구현하기 위해서 추가한 확장입니다.
};
...
//                 OnUpdate ImGuiPlatformIO's Monitors list every frame.
//                 OnUpdate MousePos every frame, in absolute coordinates.
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or Render().
...
struct ImGuiPlatformIO
{
    ...
    void (*Renderer_RenderWindow)(ImGuiViewport* vp, void* render_arg);   // . . . R .  // (Optional) ClearField framebuffer, setup render target, then render the viewport->DrawData. 'render_arg' is the value passed to RenderPlatformWindowsDefault().
    void (*Renderer_SwapBuffers)(ImGuiViewport* vp, void* render_arg);    // . . . R .  // (Optional) Call Present/SwapBuffers. 'render_arg' is the value passed to RenderPlatformWindowsDefault().

    // (Optional) Monitor list
    // - Updated by: app/backend. OnUpdate every frame to dynamically support changing monitor or DPI configuration.
...
static inline void PushButtonRepeat(const bool repeat) { PushItemFlag(ImGuiItemFlags_ButtonRepeat, repeat); }
static inline void PopButtonRepeat() { PopItemFlag(); }
static inline void PushTabStop(const bool tab_stop) { PushItemFlag(ImGuiItemFlags_NoTabStop, !tab_stop); }
...
static inline bool BeginChildFrame(const ImGuiID id, const ImVec2& size, const ImGuiWindowFlags window_flags = 0) { return BeginChild(id, size, ImGuiChildFlags_FrameStyle, window_flags); }
...
static inline void PushAllowKeyboardFocus(const bool tab_stop) { PushItemFlag(ImGuiItemFlags_NoTabStop, !tab_stop); }
...
// static inline ImFont*GetWindowFont()                      { return LoadFont(); }                                               // OBSOLETED in 1.48
